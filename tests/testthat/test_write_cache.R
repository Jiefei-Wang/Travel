...
test_that("write to an integer cache",{
    ...
    expect_equal(C_get_altptr_cache(x)$shared, FALSE)
    
    ## The read operation on x should not change
    ## the cache
    x_cache_info <- C_get_altptr_cache(x)
    x_sum <- sum(x)
    expect_equal(C_get_altptr_cache(x), x_cache_info)
    ...
    expect_equal(C_get_altptr_cache(x)$block.id, 0)
    expect_equal(C_get_altptr_cache(x)$shared, TRUE)
    expect_equal(C_get_altptr_cache(y)$block.id, 0)
    expect_equal(C_get_altptr_cache(y)$shared, TRUE)
    expect_equal(C_get_altptr_cache(x)$ptr, 
                 C_get_altptr_cache(y)$ptr)
    
    y_cache_info <- C_get_altptr_cache(y)
    ...
    ## The read operation on x should not change
    ## the cache
    x_cache_info2 <- C_get_altptr_cache(x)
    x_sum <- sum(x)
    expect_equal(C_get_altptr_cache(x), x_cache_info2)
    
    ## Same for y
    y_sum <- sum(y)
    expect_equal(C_get_altptr_cache(y), y_cache_info)
    ...
    expect_equal(C_get_altptr_cache(x)$ptr[1], y_cache_info$ptr)
    expect_equal(C_get_altptr_cache(x)$block.id, c(0,1))
    expect_equal(C_get_altptr_cache(x)$shared, c(FALSE, FALSE))
})
