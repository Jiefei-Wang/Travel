...
test_that("integer to double",{
    ...
    y[] <- rep(1L:4L, 512)
    y1 <- as.numeric(y)
    expect_true(nrow(C_get_altmmap_cache(y))==2)
    expect_true(nrow(C_get_altmmap_cache(y1))==4)
...
test_that("double to integer",{
    ...
    y[] <- rep(1:4, 512)
    y1 <- as.integer(y)
    expect_true(nrow(C_get_altmmap_cache(y))==4)
    expect_true(nrow(C_get_altmmap_cache(y1))==2)
    expect_true(all(y == rep(1L:4L, 512)))
    expect_true(all(y1 == rep(1L:4L, 512)))
})
