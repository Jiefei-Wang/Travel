...
void Filesystem_cache_copier::copy(size_t dest_index, size_t source_index)
{
    ...
    size_t source_data_offset = source_index * source_file_info.unit_size;
    size_t dest_data_offset = dest_index * dest_file_info.cache_size;
    //Load the cache
    load_source_cache(source_data_offset);
    load_dest_cache(dest_data_offset);
    //The id of the cache which stores the data of the vector element
    size_t source_cache_id = source_file_info.get_cache_id(source_data_offset);
    size_t dest_cache_id = dest_file_info.get_cache_id(dest_data_offset);
    //The offset of the beginning of the cache block in the file measured in byte
    size_t source_cache_offset = source_file_info.get_cache_offset(source_cache_id);
    size_t dest_cache_offset = dest_file_info.get_cache_offset(dest_cache_id);
...
void Filesystem_cache_copier::load_dest_cache(size_t dest_data_offset)
{
    ...
    {
        dest_cache_ptr = (char*)load_cache(dest_file_info, dest_cache_id);
