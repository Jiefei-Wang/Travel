...
#define GET_NAME(x) VECTOR_ELT(x,NAME_SLOT)
//File handle external pointer
#define GET_HANDLE_EXTPTR(x) VECTOR_ELT(x,FILE_HANDLE_SLOT)
//File handle itself
#define GET_FILE_HANDLE(x) ((file_map_handle *)R_ExternalPtrAddr(GET_HANDLE_EXTPTR(x)))
#define GET_SIZE(x) VECTOR_ELT(x,SIZE_SLOT)
#define GET_LENGTH(x) VECTOR_ELT(x,LENGTH_SLOT)

#define SET_NAME(x,v) SET_VECTOR_ELT(x,NAME_SLOT,v)
#define SET_HANDLE_EXTPTR(x,v) SET_VECTOR_ELT(x,FILE_HANDLE_SLOT,v)
#define SET_SIZE(x,v) SET_VECTOR_ELT(x,SIZE_SLOT,v)
#define SET_LENGTH(x,v) SET_VECTOR_ELT(x,LENGTH_SLOT,v)
...
#define GET_PROPS1(x) ((Rcpp::List)R_altrep_data2(x))
#define GET_NAME1(x) (x[NAME_SLOT])
#define GET_HANDLE_EXTPTR1(x) (x[FILE_HANDLE_SLOT])
#define GET_FILE_HANDLE1(x) ((file_map_handle *)R_ExternalPtrAddr(GET_HANDLE_EXTPTR1(x)))
#define GET_SIZE1(x) (x[SIZE_SLOT])
#define GET_LENGTH1(x) (x[LENGTH_SLOT])
#define GET_ALT_NAME1(x) (GET_NAME1(GET_PROPS1(x)))
#define GET_ALT_HANDLE_EXTPTR1(x) (GET_HANDLE_EXTPTR1(GET_PROPS1(x)))
#define GET_ALT_FILE_HANDLE1(x) (GET_FILE_HANDLE1(GET_PROPS1(x)))
#define GET_ALT_SIZE1(x) (GET_SIZE1(GET_PROPS1(x)))
#define GET_ALT_LENGTH1(x) (GET_LENGTH1(GET_PROPS1(x)))
...
SEXP Travel_make_altrep(int type, size_t length, file_data_func read_func, void *data, unsigned int unit_size, SEXP protect)
{
    ...
    SET_NAME(altptr_options,Rcpp::wrap(file_name));
    ...
    SET_HANDLE_EXTPTR(altptr_options,handle_extptr);
    return result;
}
